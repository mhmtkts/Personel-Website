const translations = {
  en: {
    ...
    productList: "Product List is a dynamic product listing platform that allows users to discover products by filtering them according to category, price and search criteria. This project is developed with modern web technologies and offers fast loading times, responsive design and an interface that prioritizes user experience. It is equipped with features such as lazy loading, debounce search, SPA redirection and performance optimizations. Product List is a project that showcases web development capabilities by effectively providing data visualization and user interaction.",
    budgetTracker: "A comprehensive budget tracking application built with React and Redux. Features include transaction categorization with custom tags, dark/light mode theme switching, budget limit warnings with notifications, date-based logging, and interactive charts for expense analysis. Implements Redux Toolkit for state management, React Hook Form for validation, and Tailwind CSS for responsive design. The app focuses on providing a seamless user experience with real-time updates and local storage persistence."
  },
  tr: {
    name: "Mahmut Aktaş",
    ...
    productList: "Product List, kullanıcılara ürünleri kategori, fiyat ve arama kriterlerine göre filtreleyerek keşfetme imkanı sunan dinamik bir ürün listeleme platformudur. Bu proje, modern web teknolojileri ile geliştirilmiş olup, hızlı yükleme süreleri, duyarlı tasarım ve kullanıcı deneyimini ön planda tutan bir arayüz sunar. Lazy loading, debounce arama, SPA yönlendirme ve performans optimizasyonları gibi özelliklerle donatılmıştır. Product List, etkili bir şekilde veri görselleştirme ve kullanıcı etkileşimini sağlayarak web geliştirme yeteneklerini sergileyen bir projedir.",
    budgetTracker: "React ve Redux ile oluşturulmuş kapsamlı bir bütçe takip uygulaması. Özel etiketlerle işlem kategorilendirme, karanlık/aydınlık mod tema değiştirme, bildirimli bütçe limit uyarıları, tarih bazlı kayıt tutma ve gider analizi için interaktif grafikler gibi özellikler içerir. Durum yönetimi için Redux Toolkit, form doğrulama için React Hook Form ve duyarlı tasarım için Tailwind CSS kullanır. Uygulama, gerçek zamanlı güncellemeler ve yerel depolama kalıcılığı ile sorunsuz bir kullanıcı deneyimi sunmaya odaklanır."
  },
};
