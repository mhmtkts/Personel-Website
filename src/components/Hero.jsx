...
const Hero = ({projectsRef}) => {
    ...
    return (
        <div className={`relative md:h-screen min-h-[40rem] max-h-[80rem]`} style={backgroundStyle}>
            <div className="max-sm:static max-sm:transform-none max-sm:py-24 max-sm:pb-24 max-sm:m-auto absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 max-w-4xl w-[92%] md:pl-16">
                <h1 className={`max-sm:text-[2rem] sm:text-[2.5rem] md:text-[3rem] lg:text-[3.5rem] block uppercase tracking-[3px] text-center ${darkMode ? "text-white" : "text-[#111]"} font-extrabold `}>
                    {translations.name}
                </h1>
                <div>
                    <p className={`max-sm:text-base sm:text-lg md:text-xl lg:text-[1.3rem] block my-[1em] mx-0 [unicode-bidi:isolate] ${darkMode ? "text-white" : "text-[#333333]"} text-center w-full leading-[1.6] px-4 md:px-8`}>
                        {translations.journey}
                    </p>
                </div>
                <div className='mt-20 text-center'>
                    <button onClick={scrollToProjects} className='py-4 px-16 text-xl bg-[#7843E9] text-white uppercase tracking-[1px] inline-block font-bold rounded-[5px] shadow-[0_5px_15px_rgba(0,0,0,0.15)] transition-transform duration-300 hover:-translate-y-[3px]'>
                        {translations.projects}
                    </button>
                </div>
            </div>
            <div className="hidden md:block absolute top-1/2 left-4 transform -translate-y-1/2 shadow-[0px_7px_29px_0px_rgba(100,100,111,0.2)] p-2 z-10">
